			...
			for(int k=0;k<m;k++){
				...
				for(int task=0;task<tasks;task++){
					...
					tabu.get(k).put(task,vmIndexChosen);
                             //           System.out.println(execTimes.get(task));
	...
	chooseVM(Map<Integer,Double> execTimes, Map<Integer,Double> pheromones, Map<Integer,Integer> tabu){
		...
		for(int i=0;i<pheromones.size();i++){
			if(!tabu.containsValue(i)){
				...
				probab.put(i,p);
				denominator+=p;
//                                System.out.println("prob"+probab);
//                                System.out.println("prob"+execTimes);
//                                System.out.println("prob"+pheromones);
			}
			else
				probab.put(i,0.0);
		}
